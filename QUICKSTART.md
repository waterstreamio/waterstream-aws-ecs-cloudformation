...
There are 3 CloudFormation templates for this - `commons_template.yml` creates basic network infrastructure, 
`kafka_template.yml` starts Kafka cluster in AWS MSK (if you don't already have Kafka) 
and `waterstream_template.yml` actually starts Waterstream and its auxilary infrastructure (testbox, monitoring).
